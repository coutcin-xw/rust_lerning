...
在深入每个特性之前，先建立本章四种核心语法的全景印象：

```rust
// 1. 变量声明：let 变量名[: 类型] = 初值;
let x = 5;              // 类型推断
let y: i32 = 10;        // 显式类型标注
let mut z = 0;          // mut 允许修改
const MAX: u32 = 100;   // 编译期常量，必须标注类型

// 2. 函数定义：fn 函数名(参数: 类型) -> 返回类型 { 体 }
fn add(x: i32, y: i32) -> i32 {
    x + y    // 最后一行不加分号 = 返回值
}

// 3. 表达式 vs 语句：表达式产生值，语句不产生值
let v = {               // 块也是表达式，值为最后一行
    let a = 1;
    a + 2               // 没有分号 → 返回 3
};                      // v = 3

// 4. 控制流：条件必须为 bool，循环有 loop/while/for 三种
if x > 0 { "正数" } else { "非正数" };  // if 是表达式
for i in 0..5 { }        // 范围迭代
loop { break; }           // 无限循环
while x > 0 { }           // 条件循环
```

这四个骨架组合出 Rust 所有基础代码。下面逐一展开。
