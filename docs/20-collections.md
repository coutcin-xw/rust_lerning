...
Rust 提供多种集合类型，因为不同的数据结构针对不同的访问模式做了优化——不存在一种万能的容器。

**所有权是关键区分**：`Vec<T>`、`HashMap<K,V>`、`String` 这些是*拥有型集合*，在堆上分配并持有数据的所有权；而 `&[T]`、`&str` 是*借用型切片*，只是已有数据的视图。当你需要构建并持有数据时使用拥有型集合；当你只需要读取、搜索或分片时使用切片引用。

```rust
let owned: Vec<i32> = vec![1, 2, 3];      // 堆上分配，拥有数据
let view: &[i32] = &owned[..2];           // 借用视图，不拥有
```
...
| 类型 | 核心用途 |
|------|---------|
| `Vec<T>` | 动态数组，按索引访问（O(1)），尾部增删高效 |
| `HashMap<K,V>` | 按键快速查找（O(1) 平摊），无需顺序 |
| `BTreeMap<K,V>` | 按键排序存储，支持范围查询 |
| `VecDeque<T>` | 头尾高效增删（O(1)），双端队列 |
| `BinaryHeap<T>` | 自动维护最大/最小值，优先级队列 |
| `HashSet<T>` / `BTreeSet<T>` | 去重：无须排序用 `HashSet`，需排序用 `BTreeSet` |

**决策原则**：90% 的场景从 `Vec` 开始，键值存储用 `HashMap`。只有当需要特定的访问模式（排序遍历、头尾操作、优先级）时才换用其他集合。性能上，`Vec` 的缓存局部性最好，始终是首选；`HashMap` 在大量键值场景下平替；`LinkedList` 几乎不要用——`VecDeque` 在所有方面都优于它。
