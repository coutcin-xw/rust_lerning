...
宏名以 `!` 调用（`宏名!()`），这是 Rust 区分宏调用和函数调用的方式。

### 变量捕获：`$name:type`

模式中用 `$变量名:类型` 捕获输入中的 token：

```rust
macro_rules! say {
    ($msg:expr) => {             // $msg 捕获一个表达式
        println!("你说: {}", $msg);
    };
}

say!("你好");           // 你说: 你好
say!(1 + 2);           // 你说: 3  ($msg 捕获了表达式 1 + 2)
```

在展开体中用 `$变量名`（不带冒号类型）引用捕获的值。

### 匹配多种模式

一个宏可以有多条规则，编译器选**第一条匹配的**：
...
macro_rules! my_vec {
    ...
    ( $( $x:expr ),* $(,)? ) => {
        {
            let mut v = Vec::new();
            $( v.push($x); )*    // 对匹配到的每个 $x 都展开 v.push($x);
            v
        }
    };
}
