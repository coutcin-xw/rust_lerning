...
属性（Attributes）是 Rust 的编译期指令系统。通过 `#[...]` 语法，你向编译器传递元数据和控制信号——告诉编译器"这段代码该怎样编译"，而不是"这段代码运行时该做什么"。属性的作用域涵盖：内联优化、内存布局（`repr`）、lint 规则、文档生成、trait 自动实现（`derive`）等，全部在编译阶段生效，不影响运行时行为。

`#[...]` 有两种位置写法：写在目标项上方的**外部属性**（outer attribute，也称项级属性）和写在模块/crate 内部的 `#![...]` 形式——**内部属性**（inner attribute），后者作用于整个封闭作用域而非单个项。

`#[...]` 语法在 Rust 中对应两种本质不同的机制，本章只讨论第一种：

| | 内置属性（本章） | 属性宏（第 19 章） |
|---|---|---|
| 本质 | 编译器内置指令 | 用户定义的过程宏 |
| 例子 | `#[inline]`、`#[derive(Debug)]`、`#[repr(C)]` | `#[tokio::main]`、`#[serde(rename = "...")]` |
| 定义 | Rust 语言/标准库提供 | 第三方 crate 通过 proc-macro 定义 |
