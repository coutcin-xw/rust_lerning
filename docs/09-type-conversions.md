...
在 C/C++ 中，类型转换是隐式的——`int` 自动转 `float`，`char` 自动转 `int`。这方便但危险：精度丢失、符号扩展、截断等问题在编译期沉默，运行时难以追踪。

Rust 的选择是：**一切类型转换都必须显式声明**。没有"悄悄的"类型转换。这主要由两套机制实现：

| 机制 | 用于 | 典型 trait |
|------|------|-----------|
| 类型转换 trait | 在你的类型和其他类型之间建立转换关系 | `From`/`Into`、`TryFrom`/`TryInto`、`AsRef` |
| 运算符重载 | 让你的类型支持 `+`、`-`、`[]` 等运算符 | `Add`、`Sub`、`Index` |

两者本质上都是 **trait 实现**——Rust 没有"运算符重载"这个独立特性，运算符就是 trait 的语法糖。理解了第 8 章，这一章就是 trait 的两个具体应用。

> 💡 本章是第 8 章的自然延伸——类型转换和运算符重载都是"给已有类型实现特定 trait"的实战案例。
